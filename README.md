...
`G0..G7` 现在按**三节杆绝对朝向**定义（而不是直接按关节角）：
- bit0 对应 `link1`，bit1 对应 `link2`，bit2 对应 `link3`
- `0` 表示朝下，`1` 表示朝上

例如：
- `G0 (000)`：三节都朝下
- `G1 (001)`：仅 `link1` 朝上，`link2/link3` 朝下
- `G7 (111)`：三节都朝上

环境内部会自动把上述绝对姿态换算为关节目标角 `q=[q1,q2,q3]`，避免出现“`(pi,0,0)` 实际是三节都立起”的语义混淆。
